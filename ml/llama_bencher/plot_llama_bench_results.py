#!/usr/bin/env python3
"""
Plot llama-bench Results

Creates visualizations of llama-bench performance across different prompt sizes.
...
def load_results(results_dir: str = "results") -> pd.DataFrame:
    """Load all llama-bench result CSV files (both pp and tg)."""
...
def create_prompt_size_performance_chart(
    ...
    title_parts = [
        f"llama-bench Performance: Tokens/Second vs Prompt Size{test_type_label}"
    ]
    if filter_model:
    ...
    output_file = output_dir / f"llama_bench_prompt_size{filter_suffix}.html"
...
def create_hardware_comparison_chart(
    ...
    title_parts = [f"llama-bench: {model_name_for_title}{test_type_label}"]
    if filter_quant:
        title_parts.append(f"Quantization: {filter_quant}")
    ...
    output_file = output_dir / f"llama_bench_hardware_comparison{filter_suffix}.html"
    fig.write_html(str(output_file))
    print(f"✓ Chart saved: {output_file}")
...
def main():
    ...
    if not args.summary_only:
        ...
        if args.test_type in ["pp", "both"]:
            ...
            charts_generated.append("llama_bench_prompt_size_pp.html")
            ...
            charts_generated.append("llama_bench_hardware_comparison_pp.html")
        ...
        if args.test_type in ["tg", "both"]:
            ...
            charts_generated.append("llama_bench_prompt_size_tg.html")
            ...
            charts_generated.append("llama_bench_hardware_comparison_tg.html")

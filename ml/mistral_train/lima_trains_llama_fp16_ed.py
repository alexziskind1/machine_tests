...
sys.path.append('/Users/Ext.Edmond_Jacoupeau/CORP-GIS-DTect/Scheduler/MachineLearning/SimRiskEstimation/Scripts')
import numpy as np
import pandas as pd
...
def parse_args():
   ...
   parser.add_argument("-l" , "--local_files_only",
                          help="Use locally downloaded vilt folder",
                          type=bool, 
                          default=False)
   ...
   parser.add_argument("-d", "--pytorch_device", required=True,
                          help='PyTorch device, cuda or cpu for example', 
                          default="cuda")
...
class Model(nn.Module):
    def __init__(self, local_files_only=False, cache_dir='./outputs/'):
        ...
        trans_args = {'pretrained_model_name_or_path': "openlm-research/open_llama_3b", 
    ...
    def forward(self, toks):
        ...
        x = torch.cat([self.adapation_prompt[None], x], dim=1) #  / 10
...
def conv_to_tokens(conv, tok):
    ...
    return tok.encode(text)[:2048] # Replace eos by eot

def find_targets(toks, eot_id):
    targets = []
    robot_is_talking = False
    for i, tok in enumerate(toks):
        if toks[i-4:i] == [586, 4765, 5972, 31871]:
            robot_is_talking = False
        if toks[i-4:i] == [31854, 2920, 10757, 31871]:
...
#     dataset_dir = '/Users/Ext.Edmond_Jacoupeau_SGS/'
#     pytorch_device = 'cpu'
#     val_proportion = 0.17

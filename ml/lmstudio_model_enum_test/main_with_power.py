...
class PowerMonitor:
    ...
    def _authenticate_sudo(self):
        ...
        try:
            ...
            print(
                "Failed to authenticate sudo access. Power monitoring will be disabled.",
                file=sys.stderr,
            )
            return False
                ...
                if result.returncode == 0:
                    ...
                    if power_value is not None:
                        self.power_readings.append(
                            {"timestamp": time.time(), "gpu_power_watts": power_value}
                        )

                elif (
                    result.returncode == 1
                    and "sudo: a password is required" in result.stderr
                ):
                    print(
                        "Warning: sudo authentication expired. Stopping power monitoring.",
                        file=sys.stderr,
                    )
                    break
                else:
                    print(
                        f"Warning: powermetrics failed with return code {result.returncode}: {result.stderr}",
                        file=sys.stderr,
                    )
        ...
        try:
            ...
            for line in lines:
                ...
                if line_stripped.startswith("GPU Power:"):
                    match = re.search(
                        r"GPU Power:\s*(\d+(?:\.\d+)?)\s*mW", line_stripped
                    )
                    if match:
                        gpu_power = float(match.group(1)) / 1000.0  # Convert mW to W
                        break
                    match = re.search(
                        r"GPU Power:\s*(\d+(?:\.\d+)?)\s*W", line_stripped
                    )
                    if match:
            ...
            for line in lines:
                ...
                if "gpu power" in line_lower and (
                    "mw" in line_lower or "w" in line_lower
                ):
            ...
            for line in lines:
                ...
                if ("cpu power" in line_lower or "package power" in line_lower) and (
                    "mw" in line_lower or "w" in line_lower
                ):
                    match = re.search(r"(\d+(?:\.\d+)?)\s*mw", line_lower)
                    if match:
        ...
        while self.monitoring:
            try:
                ...
                if result.returncode == 0:
                    ...
                    print(
                        f"Warning: nvidia-smi failed: {result.stderr}", file=sys.stderr
                    )
    ...
    def start_monitoring(self):
        ...
        if self.system == "Darwin":  # macOS
            ...
            if not self._authenticate_sudo():
                print(
                    "Warning: Cannot authenticate sudo access. Power monitoring disabled."
                )
                return
            ...
            print(
                f"Warning: Power monitoring not supported for GPU type: {self.gpu_type}"
            )
            return
    ...
    def stop_monitoring(self):
        ...
        powers = [
            r["gpu_power_watts"]
            for r in self.power_readings
            if r["gpu_power_watts"] is not None
        ]
    ...
    def test_powermetrics(self):
        ...
        try:
            ...
            result = subprocess.run(
                ["powermetrics", "--sample-count", "1", "--sample-rate", "1000"],
                capture_output=True,
                text=True,
                timeout=10,
            )
...
def list_models():
    ...
    if not llm_entries:
        print(
            "No LLMs found in `lms ls --json`. Have you installed any?", file=sys.stderr
        )
        sys.exit(1)
    ...
    for e in llm_entries:
        ...
        print(
            f"  • {model_name} → load as {cli_id}; params={params or 'N/A'}B; "
...
def unload_all():
    ...
    if result.returncode != 0:
        if "No models to unload" in result.stdout:
            print("  ℹ No models to unload.")
        else:
            print(f"  ⚠️  lms unload failed: {result.stderr.strip()}", file=sys.stderr)
...
def main():
    parser = argparse.ArgumentParser(
        description="Benchmark LLMs via LM Studio with power monitoring"
    )
    parser.add_argument(
        "--first-model",
        action="store_true",
    ...
    parser.add_argument(
        "--no-power", action="store_true", help="Disable power monitoring"
    )
    parser.add_argument(
        "--test-power", action="store_true", help="Test power monitoring setup and exit"
    )
    ...
    outfile = os.path.join(
        output_dir, f"benchmark_power_{mi['host']}_{mi['cpu']}_{gpu_part}.csv"
    )
    ...
    for m in models:
        ...
        try:
            ...
            print(
                f"  ✔ {name}: {query_stats['tokens_per_second']} TPS, {query_stats['total_tokens']} tokens{power_info}"
            )
        except Exception as e:
